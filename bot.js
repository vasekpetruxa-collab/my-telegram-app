...
// ВАЖНО: Заказы обрабатываются через web_app_data И через JSON-текст (альтернативный способ)
bot.on('message', async (ctx) => {
    ...
    if (ctx.message?.web_app_data?.data) {
        console.log('⏭️ Это web_app_data, пропускаем (обрабатывается отдельным обработчиком)');
