...
bot.use(async (ctx, next) => {
    try {
        ...
        await next();
        console.log('✅ Middleware завершен');
        
        // Проверяем, было ли обработано обновление с web_app_data
        if (ctx.updateType === 'message' && ctx.message?.web_app_data?.data) {
            console.log('🔍 Проверка: web_app_data должен был быть обработан обработчиком bot.on(message:web_app_data)');
            console.log('🔍 Если вы видите это сообщение, но не видите сообщения от обработчика, значит обработчик не сработал');
        }
    } catch (error) {
